...
2. 内部链接：网站内部之间的相互链接，直接链接内部页面名称即可，如： `<a href="04.3-新闻.html">新闻</a>`
3. 空链接：如果当没有确定链接目标时： `<a href="#">NULL</a>`
4. 下载链接：如果href 里面地址是一个文件或者压缩包，会下载这个文件 `<a href="文件.zip">Download</a>`
5. 网页元素链接：在网页中的各种网页元素，如文本、图像、表格、音频、视频等都可以添加超级链接
...
在 HTML 中，一个完整的表单通常是由：**表单域**、**表单控件（也成为表单元素）**和 **提示信息** 3部分构成
